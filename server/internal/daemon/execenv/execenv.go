...
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	if params.Provider == "codex" {
		...
		if err := prepareCodexHomeWithOpts(codexHome, CodexHomeOptions{CodexVersion: params.CodexVersion, CodexPath: params.CodexPath, IsLocalDirectory: params.LocalWorkDir != "", SessionStoreKey: codexSessionStoreKey(params.Profile, params.Task.AgentID, params.Task.IssueID), WritableRoots: writableRoots}, logger); err != nil {
...
func Reuse(params ReuseParams, logger *slog.Logger) *Environment {
	...
	if params.Provider == "codex" {
		...
		env.TaskHome = taskHome
		if err := prepareCodexHomeWithOpts(codexHome, CodexHomeOptions{CodexVersion: params.CodexVersion, CodexPath: params.CodexPath, ResumeSessionID: params.ResumeSessionID, IsLocalDirectory: params.LocalDirectory, SessionStoreKey: codexSessionStoreKey(params.Profile, params.Task.AgentID, params.Task.IssueID), WritableRoots: writableRoots}, logger); err != nil {
